...
describe('POST /api/score', () => {
  ...
  it('does not send a defense session to generic scoring', async () => {
    ...
    const response = await POST(new NextRequest('http://localhost/api/score', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ sessionId: 's1' }) }) as unknown as NextRequest);
