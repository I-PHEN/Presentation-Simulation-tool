  ...
  try {
    ...
    if (session.customConfig) {
      try {
        ...
        console.error('Error parsing session customConfig in stream/route:', err);
      }
    }
    ...
    const readableStream = new ReadableStream({
      async start(controller) {
        try {
          ...
          // Save complete answer in db with active speaker label
          if (fullText.trim()) {
            await db.message.create({
              data: {
                sessionId,
                role: 'assistant',
                content: `[${judgeTitle || 'Audience'}]: ${fullText.trim()}`,
              },
            });
          }
        } catch (err) {
          console.error('Error in streaming completion:', err);
        } finally {
          controller.close();
        }
      },
    ...
    console.error('Error in streaming multi-chat:', error);
