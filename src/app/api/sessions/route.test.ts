...
describe('GET /api/sessions', () => {
  ...
  it('returns only UI-safe defense sessions in newest-first order and omits malformed persisted JSON', async () => {
    findMany.mockResolvedValue([
      { id: 'new', title: 'Current deck', createdAt: new Date('2026-07-18T12:00:00Z'), status: 'completed', practiceMode: 'defense', mode: 'mock', stance: 'rigorous', deckContext: JSON.stringify({ sourceName: 'deck.pdf', slides: [{ index: 1, text: 'Claim', imageUrl: 'slide.png' }] }), findings: JSON.stringify([{ title: 'Evidence gap', risk: 'high', basis: 'response_explanation', presenterQuote: 'Claim', evidence: 'Support is missing.', slideIndex: 1, drill: 'Explain the evidence.' }]), summary: JSON.stringify({ defenseReport: { highestLeverage: { title: 'Evidence gap', risk: 'high', basis: 'response_explanation', presenterQuote: 'Claim', evidence: 'Support is missing.', slideIndex: 1, drill: 'Explain the evidence.' }, evidenceTrail: [{ slideIndex: 1, slideClaim: 'Claim', presenterSpeech: 'Claim', responseGap: 'Support is missing.', drill: 'Explain the evidence.' }], strengths: [], slideReliance: { available: true, summary: 'Speech available.', evidence: [] }, nextDrill: 'Explain the evidence.' } }), transcriptSegments: '["private"]', examinerEvents: '["private"]', scores: { overall: 99 } },
    ...
    expect(body.sessions).toEqual([
      expect.objectContaining({ id: 'new', title: 'Current deck', mode: 'mock', stance: 'rigorous', deck: { sourceName: 'deck.pdf', slides: [{ index: 1, text: 'Claim', imageUrl: 'slide.png' }] }, finding: { title: 'Evidence gap', evidence: 'Support is missing.', drill: 'Explain the evidence.' } }),
