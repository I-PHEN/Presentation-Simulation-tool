...
Provide 1-3 findings. Each finding must name a valid slide, include a quoted presenter source in presenterQuote as an exact direct quote from that slide's presenter speech, identify the response gap, and give one short drill. Use slide_reliance only where deterministic reading evidence for that same slide has actual speech and copied phrases/overlap; otherwise use response_explanation. An examiner event may be linked only when present above. Never judge slide reading without the deterministic evidence. Do not output camera, multi-judge, general scores, readiness, or generic coaching.`;
}
